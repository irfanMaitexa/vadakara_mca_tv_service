...
class CustomerOrderBookingList extends StatefulWidget {
  const CustomerOrderBookingList({Key? key}) : super(key: key);

  @override
  State<CustomerOrderBookingList> createState() =>
      _CustomerOrderBookingListState();
}

class _CustomerOrderBookingListState
    extends State<CustomerOrderBookingList> {
  @override
  Widget build(BuildContext context) {
    
    return FutureBuilder<dynamic>(
      future: ApiServices().fetchUserOrders(DbService.getLoginId()!),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          print(snapshot.data);
          return bookings.isEmpty ?  Center(child: Text('No order yet'),)  : Column(
            children: [
              AppBar(
